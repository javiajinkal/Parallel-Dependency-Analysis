...
/*
  ...
  Required files
    - Parser.h, Parser.cpp, ScopeStack.h, ScopeStack.cpp, AST.cpp, AST.h
	  ...
	  ConfigureParser.h, Utilities.h, Utilities.cpp
...
private:
  ...
  static std::mutex m;
protected:
	static std::unordered_map<std::string, Repository*> Instances;
...
public:
  ...
  static TypeTable retrieveTable() { return finalTypeTable; }
  static void addInstance(std::string fileName, Repository *repo)
  {
	  std::lock_guard<std::mutex> lock(m);
	  Instances[fileName] = repo;
  }
  TypeTable* retrieveTypeTable()
  {
	  return typetable;
  }
  ...
  static void FinalTypeTable() {
	  std::unordered_map<std::string, Repository*>::iterator it;
	  for (it = Instances.begin(); it != Instances.end(); it++) {
		  TypeTable *partialTable = (it->second)->retrieveTypeTable();
		  finalTypeTable.insert(partialTable);
	  }
  }
};
...
public:
	...
	void doAction(ITokCollection*& pTc)
	{
		...
		elem->type = "class";
		elem->name = name;
		// check if the top of scopeStack is of type namespace or name global 
		element* stacktop = p_Repos->scopeStack().top();
		//if (stacktop->type == "Namespace" || stacktop->name == "Global") {
			std::string ns = p_Repos->scopeStack().top()->name;
			p_Repos->retrieveTT()->addRecord(elem->name, elem->type, p_Repos->getFileName(), ns);
		//}
		elem->startCount = p_Repos->lineCount();
		p_Repos->scopeStack().top()->insertChild(elem);
		p_Repos->scopeStack().push(elem);
...
///////////////////////////////////////////////////////////////
// action to send semi-expression that starts a struct def
// to console
...
//==================================
//////////////////////////////////////////////////////////////
// Rule to detect beginning of typedef
...
///////////////////////////////////////////////////////////////
// action to push typedef statement to repo
...
///////////////////////////////////////////////////////////////
// action to send semi-expression that starts a typedef def
// to console
...
public:
	...
	void doAction(ITokCollection*& pTc)
	{
		//std::cout << "\n  Typedef: " << pTc->show().c_str();
	}
};

//////////////////////////////////////////////////////////////
// Rule to detect beginning of alias
...
///////////////////////////////////////////////////////////////
// action to push alias statement to repo
...
///////////////////////////////////////////////////////////////
// action to send semi-expression that starts a alias def
// to console

class PrintAlias : public IAction
{
	Repository* p_Repos;
public:
	PrintAlias(Repository* pRepos)
	{
		p_Repos = pRepos;
	}
	void doAction(ITokCollection*& pTc)
	{
		//std::cout << "\n  Alias: " << pTc->show().c_str();
	}
};
...
public:
  ...
  void doAction(ITokCollection*& pTc)
  {
    ...
    element* elem = new element;
    elem->type = "function";
	// check if its constructor or destructor
	if ((pTc->find("::") < pTc->length()) && ((((*pTc)[pTc->find("::")+1]) == (*pTc)[pTc->find("::")-1]) || (((*pTc)[pTc->find("::")+2]) == (*pTc)[pTc->find("::") - 1])))
	{
		if (pTc->find("~") == (pTc->find("::")+1)){
			elem->type = "destructor";
			elem->name = (*pTc)[pTc->find("::") - 1] + "::" + "~" + name;
		}
		else { 
			elem->type = "constructor";  
			elem->name = (*pTc)[pTc->find("::") - 1] + "::" + name;
		}
	}
	else { elem->name = name; }
